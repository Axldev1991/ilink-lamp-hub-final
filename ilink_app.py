...
# =============================================================================
# BLE Lamp Hardware Address (GATT Server)
BLE_ADDRESS = "A8:D2:CD:C7:9C:AC"
# Bluetooth Classic Address (Speaker/A2DP Sink)
AUDIO_ADDRESS = "AC:9C:C7:CD:D2:A8"
# Specific GATT Characteristic for proprietary command writing
CHAR_UUID = "0000a040-0000-1000-8000-00805f9b34fb"

# UI Aesthetic Definitions (Dark/Neon Theme)
BG_COLOR = "#0F0F0F"
ACCENT = "#00F5D4"
TEXT_COLOR = "#FFFFFF"
DIM_TEXT = "#777777"
...
class ILinkApp:
    """
    Main controller class for the iLink Lamp. 
    Manages a persistent BLE connection in a background thread and a Tkinter UI in the main thread.
    """
    def __init__(self, root):
        ...
        self.queue = None           # Async command queue (Producer-Consumer pattern)
        self.last_slider_send = 0   # Rate-limiting anchor for manual slider inputs
        self.running = True         # App execution flag
        self.rainbow_active = False # Flag for continuous rainbow color loop
        self.is_on = True           # Track power state for toggle logic
        
        # Parallel Engine Initialization:
        # We run the asyncio loop in a daemon thread to prevent blocking the UI loop.
    ...
    def _run_async_loop(self):
        """Entry point for the background thread's event loop."""
        asyncio.set_event_loop(self.loop)
        self.loop.run_until_complete(self._main_async())

    async def _main_async(self):
        """Initializes the async command queue and launches the connection manager."""
        self.queue = asyncio.Queue()
        await self._connection_manager()

    def _setup_hotkeys(self):
        """Binds physical keyboard events to control methods."""
        # Lamp controls
        ...
        # OS Bluetooth controls
        self.root.bind("b", lambda e: self.bt_power_on())
        self.root.bind("n", lambda e: self.bt_power_off())

    def toggle_power(self):
        """Switches lamp between ON and OFF states using system code 0805."""
        if self.is_on:
            self.run_cmd(("01", "080500"), True) # 0x01 = System Mode, 080500 = Power Off
            self.is_on = False
        else:
            self.run_cmd(("01", "080501"), True) # 0x01 = System Mode, 080501 = Power On
            self.is_on = True

    def _adjust_brightness(self, delta):
        """Increments/decrements local slider and sends updated value to hardware."""
    ...
    def update_status(self, text, color=ACCENT):
        """Thread-safe UI update for the bottom status bar."""
        self.root.after(0, lambda: self.status_label.config(text=text, fg=color))

    async def _connection_manager(self):
        """
        Background worker that maintains the BLE connection and processes the command queue.
        Implements smart queue compaction to eliminate latency during burst updates.
        """
        while self.running:
            # Reconnection Logic
            ...
            try:
                # Dynamic timeout based on whether we need to process rainbow frames
                timeout = 0.05 if self.rainbow_active else 0.5
                try:
                    item = await asyncio.wait_for(self.queue.get(), timeout=timeout)
                    if item is None: break # Shutdown signal
                    
                    # SMART QUEUE COMPACTION:
                    # If multiple Brightness (0801) commands are queued, we discard all but the newest.
                    while self.queue.qsize() > 0:
                        ...
                        if item[1].startswith('0801') and next_item[1].startswith('0801'):
                            ...
                            # If commands differ (e.g. Power vs Color), preserve both
                            self.queue.put_nowait(next_item); break

                    if item:
                        mode, cmd_hex = item
                        pkt = "55aa" + mode + cmd_hex
                        # CHECKSUM CALCULATION: Sum of all bytes, masked to 0xFF, subtracted from 0xFF.
                        ...
                        # Minimal hardware-safe pause between GATT writes
                        await asyncio.sleep(0.08)
                except asyncio.TimeoutError:
                    if self.rainbow_active: await self._rainbow_step()
            except Exception: 
                self.client = None # Trigger reconnection on next loop iteration

    async def _rainbow_step(self):
        """Calculates a single frame of the rainbow effect using phase-shifted sine waves."""
        t = time.time() * 0.8
    ...
    def toggle_rainbow(self):
        """Toggles the mathematical color loop flag."""
    ...
    def set_intensity_only(self, brightness):
        """Forces hardware into White Mode (080501) and updates brightness level."""
        self.rainbow_active = False
        self.rainbow_btn.config(text="🌈 MODO ARCOIRIS", bg="#1A1A1A")
        self.run_cmd(("01", "080501"), True) # Ensure wake/white
        self.is_on = True
    ...
    def apply_scene(self, r, g, b, brightness, white=False):
        """Applies a lighting preset by bursting Power, Color/Mode, and Brightness commands."""
        self.rainbow_active = False
        self.rainbow_btn.config(text="🌈 MODO ARCOIRIS", bg="#1A1A1A")
        self.run_cmd(("01", "080501"), True); self.is_on = True
        if white: self.run_cmd(("01", "080903"), True) # 080903 = Mixed Natural White Logic
        else: self.run_cmd(("03", f"0802{r:02x}{g:02x}{b:02x}"), True) # 0x03 = RGB Mode, 0802 = Set RGB
        self.run_cmd(("01", f"0801{brightness:02x}"), True)
        self.bright_slider.set(brightness)

    def bt_power_on(self): 
        """Executes system command to power ON the Bluetooth adapter."""
    ...
    def bt_power_off(self): 
        """Executes system command to power OFF the Bluetooth adapter."""
        self.update_status("⚪ APAGANDO BT...", DIM_TEXT)
        threading.Thread(target=lambda: subprocess.run(["bluetoothctl", "power", "off"])).start()
    
    def audio_on(self): 
        """Attempts connection to the device's Bluetooth Classic Audio sink."""
        threading.Thread(target=lambda: subprocess.run(["bluetoothctl", "connect", AUDIO_ADDRESS])).start()
    
    def audio_off(self): 
        """Forcefully disconnects from the device's Audio sink."""
        threading.Thread(target=lambda: subprocess.run(["bluetoothctl", "disconnect", AUDIO_ADDRESS])).start()

    def on_close(self):
        """Handles a clean shutdown by signalling background threads before exiting."""
        self.running = False
    ...
    def run_cmd(self, item, force=False):
        """
        Submits a command to the background queue.
        Implements software-side rate limiting for manual slider movement (120ms gap).
        """
        if not force and item[1].startswith('0801'):
            if time.time() - self.last_slider_send < 0.12: return
            self.last_slider_send = time.time()
        # Any manual command cancels the automatic rainbow loop
    ...
    def _setup_ui(self):
        """Builds the graphical interface using standard Tkinter widgets with a modern style."""
        ...
        # System Bluetooth Management Section
        ...
        # Quick Settings Overlay (Intensity Presets)
        ...
        # Manual Lamp Controls (Power & Modes)
        ...
        # Audio Section (Bluetooth Classic Sink Management)
        ...
        # Courier Font for the Matrix-like status bar
        self.status_label = tk.Label(self.root, text="INICIALIZANDO...", bg="#050505", fg=ACCENT, font=("Courier", 10, "bold"), pady=15)
        self.status_label.pack(side="bottom", fill="x")

    def choose_color(self):
        """Launches the OS color picker and converts Hex to RGB bytes for transmission."""
    ...
    def update_brightness(self, val): 
        """Callback for the manual brightness slider."""
        self.run_cmd(("01", f"0801{int(val):02x}"))

if __name__ == "__main__":
    # Project Initialization: Single root Window with Application Object
